...
@section('title', 'Register - TOEFLin')
...
<div class="min-h-[80vh] flex items-center justify-center">
    <div class="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden">
        <div class="px-8 pt-8 pb-6 bg-gradient-to-br from-blue-600 to-indigo-700 text-white text-center">
            ...
            <p class="text-blue-100 opacity-90">Bergabung untuk memulai simulasi</p>
        </div>
        ...
        <div class="p-8">
            <form method="POST" action="{{ route('register') }}" class="space-y-6">
                ...
                <div>
                    <label for="name" class="block text-sm font-medium text-slate-700 mb-2">Nama Mahasiswa</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="nim" class="block text-sm font-medium text-slate-700 mb-2">NIM</label>
                        <input id="nim" name="nim" type="text" value="{{ old('nim') }}" required
                            class="w-full px-4 py-3 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all duration-200">
                    </div>
                    <div>
                        <label for="fakultas" class="block text-sm font-medium text-slate-700 mb-2">Fakultas</label>
                        <input id="fakultas" name="fakultas" type="text" value="{{ old('fakultas') }}" required
                            class="w-full px-4 py-3 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all duration-200">
                    </div>
                </div>

                <div>
                    <label for="prodi" class="block text-sm font-medium text-slate-700 mb-2">Program Studi</label>
                ...
                <p class="text-center text-sm text-slate-600 mt-4">
                    Sudah punya akun? <a href="{{ route('login') }}" class="text-blue-600 hover:text-blue-800 font-semibold transition-colors">Masuk di sini</a>
                </p>
            </form>
        </div>
    </div>
